...
public class OrganizationCreationTest {
	public static void main(String[] args) throws EncryptedDocumentException, IOException {
		...
		if (actualOrganization.equals(expectedOrganizationname)) {
			javaUtility.printStatement("Organization creation is Validated");
			excelUtility.setDataIntoExcel("Organization", 2, 2, "Pass");
			excelUtility.provideDataToExcelPass(IpathConstants.VTIGEREXELFILEPATH);

		} else {
			javaUtility.printStatement("Organization creation is not Validated");
			excelUtility.setDataIntoExcel("Organization", 2, 2, "Fail");
			excelUtility.provideDataToExcelPass(IpathConstants.VTIGEREXELFILEPATH);
		}
